...
public class EmployeeController {
    ...
    public String edit(@PathVariable String code, @AuthenticationPrincipal UserDetail userDetail, Model model,Employee employee) {
        if(code==null) {
    ...
    public String update(@AuthenticationPrincipal UserDetail userDetail,@Validated Employee employee, BindingResult res, Model model) {
        // 入力チェック
        if (res.hasErrors()) {
            return edit(null,userDetail,model,employee);
        }
             // パスワードが空白だった場合
        ...
        if ("".equals(employee.getPassword())) {
            // 空白の場合
            result = employeeService.updateWithoutPassword(employee);
